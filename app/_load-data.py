...
from models import Game, Publisher, Genre, User, Review 
...
def game_extract():
    ...
    with open ('../_raw-data/archive/vgsales.csv', 
        ...
        for row in reader:
            ...
            if genre_checker.match(game_genre):
                continue
            else:
                genre_set.add(game_genre)

    print(f'Game : {games_list[0]}')


def load_genres():
    genre_list = [dataclasses.asdict(Genre(genre_label=genre)) for genre in genre_set]
    print(genre_list)
    mongo.db.genre.insert_many(genre_list)
...
def load_publishers():
    ...
    publisher_list = [dataclasses.asdict(Publisher.create_publisher(publisher)) for publisher in publisher_set]
    print(f"{publisher_list[1]['name']}, {publisher_list[1]['label']}")

    mongo.db.publisher.insert_many(publisher_list) 
...
def main():
    fake_user=User.create_user(password='pass', name='fake_user', email='fake@fakemail.com')
    mongo.db.users.insert_one(dataclasses.asdict(fake_user))
