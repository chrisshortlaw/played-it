...
class BaseModel:
    """
    A basic model with a name field and will hold our to_mongo_dict
    method and our from_mongo method.
    This will permit us to manipulate class
    instances via the database model rather than dictionaries.
    Manipulating instances allows us to set and
    enforce certain data-types,
    use type hinting and
    ...
    def to_mongo_dict(self):
        """
        Instance Method converting
        dataclass to dict allowing for a smoother
        exchange between MongoDB
        and the app.
        _id is not specified on the BaseClass
        However, the method pops any _id class
        """
    ...
    def from_mongo(cls, **mongodict):
        """
        Creates a class from a return
        value from a mongo query.
        Add the _id to the class instance
        which will permit useful querying.
        Params:
...
class User(BaseModel):
    ...
    def password(self, password_string):
        ...
        """
        hashed_pass = generate_password_hash(password_string, 
    ...
    def check_pass(self, password_string):
        ...
        Checks hashed pass 
        against password_string
        and returns True if match,
        false otherwise
        """
    ...
    def create_pass(cls, password_string):
        """
        Uses werkzeug built in password hash
        to generate a hashed 
        pass and return it
        """
    ...
    def update_user_with_dict(self, db_loc=mongo.db.users, **fields):
        ...
        updated_user = db_loc.find_one_and_update({"_id": self._id},
                                                  {"$set": self.__dict__})
        return updated_user
...
class Game(BaseModel):
    ...
    @classmethod
    def create_game(cls, label, platform,
    ...
    @classmethod
    def add_game(cls, label, platform, year,
                 genre, publisher, db_loc=mongo.db.games):
        """
    ...
    def update_game(self, db_loc=mongo.db.games):
        updated_game = db_loc.find_one_and_update({"_id": self._id},
                                                  {"$set": self.__dict__})
        return updated_game
...
class Publisher(BaseModel):
    ...
    name: should have the pattern: 
    'devolver_digital, sony_computer_entertainment' -> lowercase, snake_case
...
class Review(BaseModel):
    ...
    @classmethod
    def add_review(cls, name,
                   game, game_id,
                   author, author_id,
                   text, pub_date,
                   game_ref=None,
                   author_ref=None,
                   db_loc=mongo.db.reviews):
    ...
    def update_review(self, db_loc=mongo.db.reviews):
        updated_review = db_loc.find_one_and_update({"_id": self._id},
                                                    {"$set": self.__dict__}) 
    ...
    def update_review_with_dict(self, db_loc=mongo.db.tests, **fields):
        for key, value in fields.items():
            setattr(self, key, value)
        updated_review = db_loc.find_one_and_update({"_id": self.id}, 
                                                    {"$set": self.__dict__})
        return updated_review
