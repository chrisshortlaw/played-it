...
from wtforms.validators import (DataRequired, ValidationError, 
                                EqualTo, InputRequired, Email, Length)
...
def check_username(form, field):
    message = "Username in use. Please choose a different."
...
choices = [(publisher["_id"], publisher["label"])
           for publisher in publisher_docs]
...
class UserReviewForm(FlaskForm):
    ...
    game = SelectField('game', validators=[DataRequired()],
                       choices=game_choices, render_kw={"class": "input"})
    submit = SubmitField('Add Review',
                         render_kw={"class": "button is-block is-info is-large is-fullwidth"})
        ...
        if email.data != self.current_email:
            ...
            if user is not None:
                raise ValidationError("""
                                      Email is in use. 
                                      Please choose a different email or login.
                                      """)
...
class AddGameRef(FlaskForm):
    submit = SubmitField('Add This Game',
                         render_kw={"class": "button is-medium is-primary"})


class DeleteGame(FlaskForm):
    submit = SubmitField('Delete This Game',
                         render_kw={"class": "button is-medium is-danger"})
