...
class TestUser(unittest.TestCase):
    ...
    def test_class_to_mongo(self):
        new_user = User.add_user(password='pass', name='test_user1',
        ...
        new_game = Game.add_game(label='fake game 4', platform='SNES', 
                                 year=1996, genre='Sports', 
                                 publisher=fake_pub, db_loc= mongo.db.tests)
        check_game_dict = Game.from_mongo(
                                          **(mongo.db.tests.find_one({"label": "fake game 4"})))
    ...
    def test_class_from_mongo(self):
        new_user = User.add_user(password="pass", name="test_user1", 
                                 email="test@testmail.com", db_loc=mongo.db.tests)

        fake_publisher = Publisher.create_publisher("Nintendo")
        fake_publisher.upload_publisher(db_loc=mongo.db.tests)
        new_game = Game.add_game("tekken", 'PlayStation',
                                 year=1997, genre='fighting',
                                 publisher=fake_publisher,
                                 db_loc=mongo.db.tests)
        new_review = Review.add_review(name='Good', game=new_game.name,
                                       game_id=new_game._id,
                                       author=new_user.name,
                                       author_id=new_game._id,
                                       text="Okay", pub_date='12-12-21',
                                       db_loc=mongo.db.tests)
    ...
    def test_password(self):
        new_user = User.create_user(password="pass", name="test_user",
                                    email="test@user.com")
    ...
    def test_add_user(self):
        new_user = User.add_user(password="password", name="new_user1", 
                                 email="fake@fakemail.com", db_loc=mongo.db.tests)
    ...
    def test_load_existing_user(self):
        new_user = User.create_user(password="pass", name="new_user2", 
    ...
    def test_check_user(self):
        new_user = User.add_user(password="pass", name="new_user2",
                                 email="fake2@fakemail.com", 
                                 db_loc=mongo.db.tests)
        ...
        if check_user:
            ...
            another_user = User.add_user(password="pass", name="new_user2",
    ...
    def test_check_user_email(self):
        ...
        self.assertTrue(len(check_duplicate) == 1, 
                        'Check Duplicate: Fail - Multiple Users detected.')
        session = {}
        self.assertTrue(check_password_hash(check_duplicate[0]['_password'],
                                            'pass'))
    ...
    def test_add_review(self):
        ...
        if reviewed_game is None:
            new_game = Game.create_game(label='fake game 3', platform="SNES",
                                        year=1996, genre="action",
                                        publisher=pub_doc.get("label"),
                                        publisher_id=pub_doc.get('_id'))
            mongo.db.tests.insert_one(new_game.__dict__)
            reviewed_game = mongo.db.tests.find_one({"label": "fake game 3"})

        self.assertFalse(reviewed_game is None,
                         'Reviewed_game is not present in db')
        ...
        self.assertFalse(review_author is None, 'User is not present in db')

        test_review = Review(name="Test Review",
                             game=reviewed_game['label'],
                             game_id=reviewed_game['_id'],
                             author=review_author["name"],
                             author_id=review_author["_id"],
        ...
        self.assertEqual(retrieved_review.get("author_id"),
                         review_author.get("_id"))
    ...
    def test_update_review(self):
        ...
        new_user = User.add_user(password='pass', name='test_user1',
                                 ...
                                 db_loc=mongo.db.tests)
        new_game = Game.add_game(label="fake_game3", 
                                 platform='Wii',
                                 year=2003,
                                 genre='action',
        ...
        self.assertEqual(new_review.id, review_id_check,
                         'Updt review and original ids should match. They do not.')
